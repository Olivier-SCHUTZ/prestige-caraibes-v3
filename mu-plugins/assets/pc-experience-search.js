...
(function (w, d, $) {
  ...
  $(function () {
    ...
    if ($mapContainer.length && typeof w.L !== "undefined") {
      ...
      if (
        typeof pc_exp_initial_data !== "undefined" &&
        pc_exp_initial_data.map_data
      ) {
    ...
    function updateMapMarkers(items) {
      ...
      items.forEach(function (item) {
        ...
        const popupPrice = item.price ? `${item.price}<br>` : "";
        const popupContent = `<strong>${item.title}</strong><br>${popupPrice}<a href="${item.link}" target="_blank" rel="noopener">Voir l'expérience</a>`;
        ...
        success: function (response) {
          if (response.success && response.data) {
            ...
            $resultsContainer.html(
              '<div class="pc-no-results"><h3>Une erreur est survenue.</h3><p>Veuillez réessayer.</p></div>'
            );
          }
        },
        error: function () {
          $resultsContainer.html(
            '<div class="pc-no-results"><h3>Une erreur de communication est survenue.</h3></div>'
          );
        },
        complete: function () {
    ...
    $wrapper.on("change", "#filter-exp-category, #filter-exp-ville", () =>
      performSearch(1)
    );
    $wrapper.on("keyup", "#filter-exp-keyword", debouncedSearch);
    $wrapper.on(
      "change",
      "#filter-exp-prix-min, #filter-exp-prix-max, #filter-exp-participants",
      debouncedSearch
    );
