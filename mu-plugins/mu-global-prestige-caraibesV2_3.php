  ...
  function pcseo_get_seo_description($post_id)
  {
    ...
    switch ($pt) {
      ...
      case 'post':
        ...
        break; // Articles (champ social d̩di̩)
    }
...
if (is_admin()) {
  ...
  function pcseo_render_audit_page()
  {
    ...
    foreach ($rows as $r) {
      if (++$i > 200) break;
      printf(
        '<tr><td>%s</td><td>%d</td><td>%s</td><td><a href="%s" target="_blank">%s</a></td><td>%s</td><td>%s</td><td>%s</td></tr>',
        esc_html($r['type']),
        $r['id'],
        esc_html($r['title']),
        esc_url($r['url']),
...
add_action('wp_footer', function () {
  ...
  if (!$lat || !$lng) {
    $old = $getf('coordonnees', '');
    if (is_string($old) && strpos($old, ',') !== false) {
      [$la, $lo] = array_map('trim', explode(',', $old, 2));
      if ($la !== '' && $lo !== '') {
        $lat = $la;
        $lng = $lo;
      }
    ...
    foreach ($review_posts as $rp) {
      ...
      if ($rating > 0) {
        ...
        $reviews[] = [
          ...
          'contentReferenceTime' => (function ($d) {
            return $d ? date('c', strtotime($d . '-01')) : null;
          })(get_post_meta($rp->ID, 'pc_stayed_date', true)),
        ];
      }
    }
    if ($cnt > 0) {
...
add_action('wp_footer', function () {
  ...
  if ($search_type === 'logement' && function_exists('pc_get_filtered_logements')) {
    ...
    $filters = [
      'page' => 1,
      'category' => sanitize_text_field($_GET['categorie'] ?? ''),
      'ville' => sanitize_text_field($_GET['ville'] ?? ''),
...
add_action('wp_head', function () {
  ...
  if (!function_exists('pcseo_clean_lines')) {
    ...
    {
      $out = [];
      foreach (preg_split('/\R+/', (string)$txt) as $l) {
        $l = trim($l);
        if ($l !== '') $out[] = $l;
      }
      return $out;
