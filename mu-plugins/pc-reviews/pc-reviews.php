 ...
 * Description: Module autonome d'avis clients. Gère le CPT, les Assets, le "Voir plus" en AJAX et le JSON-LD.
 */
...
add_action('wp_enqueue_scripts', function () {
    // Ne charge les assets que sur les pages de détails (où les shortcodes risquent d'être utilisés)
    if (!is_singular()) return;

    $css_path = plugin_dir_path(__FILE__) . 'assets/css/pc-reviews.css';
    if (file_exists($css_path)) {
        wp_enqueue_style('pc-reviews-css', plugin_dir_url(__FILE__) . 'assets/css/pc-reviews.css', ['pc-base'], filemtime($css_path));
    }

    $js_path = plugin_dir_path(__FILE__) . 'assets/js/pc-reviews.js';
    if (file_exists($js_path)) {
        wp_enqueue_script('pc-reviews-js', plugin_dir_url(__FILE__) . 'assets/js/pc-reviews.js', [], filemtime($js_path), true);

        // Injecte l'URL Ajax sécurisée pour notre script
        wp_localize_script('pc-reviews-js', 'pcReviewsData', [
...
add_action('manage_pc_review_posts_custom_column', function ($column, $post_id) {
    if ($column === 'related_post') {
        ...
        if ($related_id && ($related_post = get_post($related_id))) {
            ...
            echo '<strong><a href="' . esc_url($edit_link) . '">' . esc_html(get_the_title($related_id)) . '</a></strong><br><em>(' . esc_html($type_label) . ')</em>';
        } else {
            echo '—';
        }
    }
...
function pc_rev_stars_svg($rating)
{
    ...
    $out = '<div class="pc-stars" aria-label="' . esc_attr($r) . '/5" style="display:flex; gap: 4px;">';
    for ($i = 1; $i <= 5; $i++) {
        $filled = $i <= $r ? 'currentColor' : 'none';
        $out .= '<svg viewBox="0 0 24 24"><path d="M12 17.3l-6.18 3.75 1.64-7.03L2 9.77l7.19-.61L12 2.5l2.81 6.66 7.19.61-5.46 4.25 1.64 7.03z" fill="' . $filled . '" stroke="currentColor" stroke-width="1.5"/></svg>';
    }
    return $out . '</div>';
}
...
function pc_rev_get_internal_stats($post_id)
{
    ...
    foreach ($q->posts as $rid) {
        $sum += intval(get_post_meta($rid, 'pc_rating', true));
    }
...
function pc_rev_format_stayed($raw)
{
    $raw = trim((string)$raw);
    if (!$raw) return '';
    if (preg_match('/^\d{8}$/', $raw)) return substr($raw, 0, 4) . '-' . substr($raw, 4, 2) . '-' . substr($raw, 6, 2);
    if (preg_match('/^\d{6}$/', $raw)) return substr($raw, 0, 4) . '-' . substr($raw, 4, 2);
...
function pc_rev_render_card($rid)
{
    ...
    <article class="pc-review-card">
        <div class="pc-review-card-header">
            <div class="pc-review-card-author">
                <?php echo esc_html($name); ?><?php echo $loc ? ' <span>(' . esc_html($loc) . ')</span>' : ''; ?>
            </div>
            <div class="pc-review-card-stars"><?php echo pc_rev_stars_svg($rating); ?></div>
        </div>
        <?php if ($title): ?><h3><?php echo esc_html($title); ?></h3><?php endif; ?>
        <div class="pc-review-card-body"><?php echo wpautop(esc_html($body)); ?></div>
        <div class="pc-review-card-meta">
            <?php
            $tail = [];
...
add_shortcode('pc_reviews', function ($atts) {
    ...
    $nonce = wp_create_nonce('pc_reviews_more');

    ob_start(); ?>
    <section id="avis" class="pc-reviews">
        <div class="pc-reviews-head">
            <h3>Avis</h3>
            <?php if ($stats['count']): ?>
                <div class="pc-reviews-stats">
                    <strong><?php echo esc_html($stats['avg']); ?></strong>
                    <?php echo pc_rev_stars_svg(round($stats['avg'])); ?>
                    <span>(<?php echo intval($stats['count']); ?>)</span>
                </div>
            <?php endif; ?>
            <a href="#pc-review-form" class="pc-rev-toggle-form">Laisser un avis</a>
        </div>
        ...
        <?php if ($has_more): ?>
            <div style="text-align:center;margin-top:10px">
                <button type="button" class="pc-rev-more" data-post="<?php echo esc_attr($post_id); ?>" data-offset="<?php echo esc_attr($limit); ?>" data-limit="<?php echo esc_attr($limit); ?>" data-nonce="<?php echo esc_attr($nonce); ?>">
                    Voir plus
                </button>
            </div>
        <?php endif; ?>
...
add_shortcode('pc_review_form', function ($atts) {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['pc_review_nonce']) && wp_verify_nonce($_POST['pc_review_nonce'], 'pc_review_submit')) {
        ...
        if (get_transient($ipkey)) {
            $msg = '<div style="color:#b00020;margin-bottom:1rem;">Trop de tentatives, réessayez plus tard.</div>';
        } else {
            ...
            if ($name && $email && $rating && $body) {
                ...
                if ($rid && !is_wp_error($rid)) {
                    ...
                    wp_mail(get_option('admin_email'), 'Nouvel avis en attente', "Un avis a été soumis pour la fiche #$post_id.\nModérez-le dans l’admin.");
                    set_transient($ipkey, 1, 15 * MINUTE_IN_SECONDS);
                    $msg = '<div style="color:#0a7a3b;margin-bottom:1rem;">Merci ! Votre avis est bien enregistré et sera publié après validation.</div>';
                }
            } else {
                $msg = '<div style="color:#b00020;margin-bottom:1rem;">Merci de remplir les champs obligatoires.</div>';
            }
        }
    }

    ob_start(); ?>
    <form id="pc-review-form" class="pc-review-form-wrapper" method="post" style="<?php echo $open ? '' : 'display:none;'; ?>">
        <?php echo $msg; ?>
        <div class="pc-review-form-grid">
            <div class="pc-rev-field"><label>Nom*</label><input name="pc_name" required></div>
            <div class="pc-rev-field"><label>Email*</label><input type="email" name="pc_email" required></div>
            <div class="pc-rev-field"><label>Pays (optionnel)</label><input name="pc_location"></div>
            <div class="pc-rev-field"><label>Note*</label>
                <select name="pc_rating" required>
                    <option value="">—</option><?php for ($i = 5; $i >= 1; $i--) echo "<option value='$i'>$i</option>"; ?>
                </select>
            </div>
            <div class="pc-rev-field full"><label>Titre (optionnel)</label><input name="pc_title"></div>
            <div class="pc-rev-field full"><label>Votre avis*</label><textarea name="pc_body" required rows="6"></textarea></div>
            <div class="pc-rev-field full"><label>Mois de séjour (YYYY-MM)</label><input name="pc_stayed" placeholder="2025-08"></div>
        </div>
        <input type="text" name="website" style="display:none">
        <?php wp_nonce_field('pc_review_submit', 'pc_review_nonce'); ?>
        <button type="submit" style="margin-top:16px;">Envoyer mon avis</button>
    </form>
<?php return ob_get_clean();
});
...
function pc_reviews_ajax_more()
{
    ...
    if (!wp_verify_nonce($nonce, 'pc_reviews_more')) wp_send_json_error(['message' => 'Invalid nonce'], 403);
    ...
    wp_send_json_success(['html' => $html, 'nextOffset' => $nextOffset, 'hasMore' => ($nextOffset < $total)]);
}
