    ...
    <table class="pc-resa-dashboard-table">
        ...
        <tbody>
            <?php if (! empty($reservations)) : ?>
                <?php
                // [MODIF] Chargement des modèles PDF
                $pdf_templates = get_posts([
                            ...
                            <div class="pc-resa-card">
                                ...
                                <div class="pc-resa-card__body">
                                    ...
                                    <div class="pc-resa-card__section pc-resa-card__section--full" style="flex: 1 1 100%; max-width: 100%;">
                                        <h4 style="margin-bottom:15px; border-bottom:1px solid #eee; padding-bottom:5px;">📄 Documents & Facturation</h4>

                                        <div class="pc-doc-actions" style="background:#f8fafc; padding:10px; border:1px solid #e2e8f0; border-radius:6px; margin-bottom:15px; display:flex; gap:10px; align-items:center; flex-wrap:wrap;">
                                            <?php if ($pdf_templates): ?>
                                                <select class="pc-doc-template-select" style="max-width:200px; border:1px solid #ccc; border-radius:4px; padding:5px;">
                                                    <option value="">-- Choisir un modèle --</option>
                                                    <?php foreach ($pdf_templates as $tpl): ?>
                                                        <option value="<?php echo $tpl->ID; ?>"><?php echo esc_html($tpl->post_title); ?></option>
                                                    <?php endforeach; ?>
                                                </select>

                                                <button type="button" class="button button-small pc-btn-generate-doc" data-resa-id="<?php echo $resa->id; ?>" style="background:#2271b1; color:white; border:none; padding:5px 10px; border-radius:4px; cursor:pointer;">
                                                    Générer PDF
                                                </button>

                                                <label style="font-size:11px; margin-left:auto; display:flex; align-items:center; gap:4px; color:#64748b; cursor:pointer;">
                                                    <input type="checkbox" class="pc-doc-force-regen"> Forcer régénération (écraser)
                                                </label>
                                            <?php else: ?>
                                                <em style="color:#d63638; font-size:12px;">⚠️ Aucun modèle créé. Allez dans "Documents PDF" pour en créer un.</em>
                                            <?php endif; ?>
                                        </div>

                                        <div class="pc-documents-list-container" id="pc-docs-list-<?php echo $resa->id; ?>" data-resa-id="<?php echo $resa->id; ?>">
                                            <table class="wp-list-table widefat fixed striped" style="border:1px solid #e5e5e5; box-shadow:none; width:100%;">
                                                <thead>
                                                    <tr>
                                                        <th style="padding:8px; text-align:left; font-size:11px; color:#666;">TYPE</th>
                                                        <th style="padding:8px; text-align:left; font-size:11px; color:#666;">FICHIER</th>
                                                        <th style="padding:8px; text-align:left; font-size:11px; color:#666;">DATE</th>
                                                        <th style="padding:8px; text-align:right; font-size:11px; color:#666;">ACTION</th>
                                                    </tr>
                                                </thead>
                                                <tbody class="pc-docs-tbody">
                                                    <tr>
                                                        <td colspan="4" style="text-align:center; color:#999; padding:15px;">Chargement des documents...</td>
                                                    </tr>
                                                </tbody>
                                            </table>
                                        </div>
