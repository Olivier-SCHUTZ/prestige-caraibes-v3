...
/**
 * Script de migration temporaire : Copie les données ACF vers le format Natif (Expériences).
 * À lancer en allant sur : ton-site.local/wp-admin/?run_pc_migration=experiences
 */
add_action('admin_init', function () {
    // Sécurité : Uniquement si on demande la migration et qu'on est admin
    if (!isset($_GET['run_pc_migration']) || $_GET['run_pc_migration'] !== 'experiences') {
        return;
    }
    ...
    echo '<h1>🚀 Lancement de la Migration des Expériences...</h1>';

    $query = new WP_Query([
        'post_type'      => 'experience',
        'posts_per_page' => -1, // On prend TOUTES les expériences
        'post_status'    => 'any'
    ]);

    $manager = class_exists('PCR_Field_Manager') ? PCR_Field_Manager::init() : null;
    $config  = class_exists('PCR_Experience_Config') ? PCR_Experience_Config::get_instance() : null;
            ...
            if (in_array($post->post_type, $group_config['post_types'])) {
                foreach ($group_config['fields'] as $field_key => $field_config) {
                    ...
                    if ($old_value !== null && $old_value !== false && $old_value !== '') {
                        // Nettoyage : Si c'est un tableau (comme les répéteurs exp_types_de_tarifs, exp_faq, etc.), on unslash simplement
    ...
    echo "<h2>🎉 Terminé ! $count expériences ont été migrées avec succès vers le format natif.</h2>";
